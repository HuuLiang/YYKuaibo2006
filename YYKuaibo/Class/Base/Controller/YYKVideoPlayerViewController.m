...
- (void)viewDidLoad {
    ...
    [_closeButton bk_addEventHandler:^(id sender) {
        ...
        if (_shouldPopupPaymentIfNotPaid && ![YYKUtil isPaid]) {
            [[YYKPaymentViewController sharedPaymentVC] popupPaymentInView:self.view forProgram:(YYKProgram *)self.video withCompletionHandler:^{
                @strongify(self);

...
#import "YYKHomeRankingCell.h"

static NSString *const kRankingCellReusableIdentifier = @"RankingCellReusableIdentifier";

@interface YYKHomeRankingRowCell () <UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
{
...
@implementation YYKHomeRankingRowCell
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _layoutCV.backgroundColor = [UIColor whiteColor];
        _layoutCV.delegate = self;
        _layoutCV.dataSource = self;
        [_layoutCV registerClass:[YYKHomeRankingCell class] forCellWithReuseIdentifier:kRankingCellReusableIdentifier];
...
- (void)setItems:(NSArray<YYKHomeRankingRowCellItem *> *)items {
    _items = items;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    YYKHomeRankingCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kRankingCellReusableIdentifier forIndexPath:indexPath];
    
    if (indexPath.item < self.items.count) {
        YYKHomeRankingRowCellItem *item = self.items[indexPath.item];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    return CGSizeMake([YYKHomeRankingCell widthRelativeToHeight:itemHeight withImageScale:5./3.], itemHeight);
}
...
@implementation YYKHomeRankingRowCellItem
